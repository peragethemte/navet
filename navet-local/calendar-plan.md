...
- **The add-on ships the iCloud proxy but never the sidecar.** Every snippet included by
  `docker/nginx.conf` has to be copied into the add-on image with a matching `js_import`, or nginx
  refuses to start on an include it cannot open; the transit branch established that rule when it
  fixed the same breakage for Yr. The iCloud proxy follows it, so the add-on's nginx loads, but the
  Python sidecar is not in that image and the add-on has no way to supply an Apple ID
  (`config.yaml:23-26` is a single boolean). A request there returns 502, which the provider treats
  as unreachable and retries on its backoff ladder. Supporting iCloud on the add-on is its own
  change: it needs the sidecar in that image and a credentials input in the add-on options.
